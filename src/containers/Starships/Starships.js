...
class Starships extends Component {
    state = {
        data: null,
        isLoading: true,
        api: 'https://swapi.co/api/starships?format=json&page=1'
    }
        ...
        return (
            <div className="container-x container-content">
                ...
                }
                {
                    ...
                    <div>
                        <h4>Total Starships : <b>{ data.count }</b></h4>
                        <ul className="list">
                        { 
                            data.results.map((value, key) => {
                                return (
                                    <li className="list-item" key={ `${ value.name }-${ key }` }><Link to={ `/starships/${ key + 1 }` }> { value.name } </Link></li>
                                );
                            })
                        }
                        </ul>
