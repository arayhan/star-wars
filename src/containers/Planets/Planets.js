...
class Planets extends Component {
    state = {
        data: null,
        isLoading: true,
        api: 'https://swapi.co/api/planets?format=json&page=1'
    }
        ...
        return (
            <div className="container-x container-content">
                ...
                }
                {
                    ...
                    <div>
                        <h4>Total Planets : <b>{ data.count }</b></h4>
                        <ul className="list">
                        { 
                            data.results.map((value, key) => {
                                return (
                                    <li className="list-item" key={ `${ value.name }-${ key }` }><Link to={ `/planets/${ key + 1 }` }> { value.name } </Link></li>
                                );
                            })
                        }
                        </ul>
