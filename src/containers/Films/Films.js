...
class Films extends Component {
    state = {
        data: null,
        isLoading: true,
        api: 'https://swapi.co/api/films?format=json&page=1'
    }
        ...
        return (
            <div className="container-x container-content">
                ...
                }
                {
                    ...
                    <div>
                        <h4>Total films : <b>{ data.count }</b></h4>
                        <ul className="list">
                        { 
                            data.results.map((value, key) => {
                                return (
                                    <li className="list-item" key={ `${ value.title }-${ key }` }><Link to={ `/films/${ key + 1 }` }> { value.title } </Link></li>
                                );
                            })
                        }
                        </ul>
