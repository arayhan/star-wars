...
class Species extends Component {
    state = {
        data: null,
        isLoading: true,
        api: 'https://swapi.co/api/species?format=json&page=1'
    }
        ...
        return (
            <div className="container-x container-content">
                ...
                }
                {
                    ...
                    <div>
                        <h4>Total Species : <b>{ data.count }</b></h4>
                        <ul className="list">
                        { 
                            data.results.map((value, key) => {
                                return (
                                    <li className="list-item" key={ `${ value.name }-${ key }` }><Link to={ `/species/${ key + 1 }` }> { value.name } </Link></li>
                                );
                            })
                        }
                        </ul>
